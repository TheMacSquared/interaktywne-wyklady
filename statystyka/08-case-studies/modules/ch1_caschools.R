...
ch1_ui <- lecture_chapter(id = "ch1", num = "1", title = "CASchools", content = tagList(
  fluidRow(column(8, offset = 2,
    ...
    div(class = "analysis-step",
      ...
      "Sprawdźmy, czy za wielkością klas i wynikami nie kryje się wspólne tło:
        sytuacja materialna uczniów."
    ),

    div(class = "lc-prose",
      p("W danych mamy odsetek uczniów korzystających z dofinansowanych obiadów.
        Wykorzystamy go jako wskazówkę o sytuacji materialnej rodzin."),
      p(tags$em("Czy dopłaty do obiadów pogarszają oceny?")),
      p("Na wykresie więcej uczniów korzystających z pomocy idzie w parze
        z niższymi wynikami. Ale pomoc częściej trafia tam, gdzie sytuacja
        jest trudniejsza. Sama korelacja nie mówi, że obiady szkodzą —
        może pokazywać, kto potrzebuje wsparcia."),
      p("Sprawdźmy więc dwa związki: odsetek korzystających z obiadów z wynikami
        oraz z liczbą uczniów na nauczyciela. Czy sytuacja materialna pomaga
        zrozumieć pierwszy wykres?")
    ),

    div(class = "lc-figure-panel",
      h4("Obiady, wyniki i wielkość klas — co idzie ze sobą w parze?"),
      fluidRow(
        column(6,
    ...
    div(class = "lc-prose",
      p("Sytuacja materialna jest powiązana z obydwoma wynikami.
        Teraz kluczowe pytanie: czy związek STR z wynikami pozostanie,
        gdy porównamy dystrykty o podobnej sytuacji uczniów?")
    ),
    ...
    div(class = "lc-feedback lc-feedback-ok",
      ...
      tags$ol(
        tags$li(tags$b("Prosty związek STR→wyniki istnieje"),
                " (r ≈ −0.23), ale to nie cała historia:
                biedniejsze dystrykty mają też więcej uczniów na nauczyciela i gorsze wyniki."),
        ...
        tags$li(tags$b("Wydatki na ucznia mają słaby związek z wynikami w tym modelu"),
                " po uwzględnieniu sytuacji materialnej. To nie znaczy,
                że dodatkowe pieniądze nie pomagają — tak jak korelacja
                z obiadami nie znaczy, że pomoc szkodzi.")
      )
    ),

    div(class = "lc-feedback lc-feedback-warning",
      tags$strong("Co powiedzieć decydentowi:"),
      p("Pierwszy wykres to za mało, żeby obiecać, o ile poprawią się wyniki
        po zmniejszeniu klas. Gdy uwzględniamy sytuację uczniów, obraz się zmienia.
        Warto przyjrzeć się zarówno organizacji nauki, jak i potrzebom rodzin."),
      p("Zapamiętaj przykład obiadów: pomoc może występować razem z gorszymi
        wynikami właśnie dlatego, że trafia do potrzebujących. Przed decyzją
        zapytaj, co jeszcze różni porównywane grupy.")
    ),
...
ch1_server <- function(input, output, session) {
  ...
  output$ch1_conf_stats <- renderUI({
    ...
    lc_stat_grid(
      lc_stat_box("Obiady a wyniki", paste0("r = ", round(r_lunch_score, 3)),
                  color = case_highlight),
      lc_stat_box("Obiady a STR", paste0("r = ", round(r_lunch_str, 3)),
                  color = case_test),
      lc_stat_box("Wniosek", "Warto uwzględnić sytuację uczniów", color = case_conclude),
      columns = 3
    )
  })
