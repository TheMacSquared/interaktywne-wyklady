...
# Dane palmerpenguins są wczytywane w helpers.R jako .cas_data / .cas_labels
# (wspólne dla wszystkich rozdziałów regresji).

.ch1_pred_specs <- list(
  mass_flipper = list(
    label = "Masa ciała ~ długość płetwy",
    x = "flipper_length_mm", y = "body_mass_g", default = 200, step = 1,
    unit = "mm długości płetwy",
    question = "Jaka będzie przewidywana średnia masa ciała pingwina o takiej długości płetwy?"
  ),
  mass_bill_length = list(
    label = "Masa ciała ~ długość dzioba",
    x = "bill_length_mm", y = "body_mass_g", default = 45, step = 0.5,
    unit = "mm długości dzioba",
    question = "Jaka będzie przewidywana średnia masa ciała pingwina o takiej długości dzioba?"
  ),
  billlen_flipper = list(
    label = "Długość dzioba ~ długość płetwy",
    x = "flipper_length_mm", y = "bill_length_mm", default = 200, step = 1,
    unit = "mm długości płetwy",
    question = "Jaka będzie przewidywana średnia długość dzioba przy takiej długości płetwy?"
  ),
  mass_bill_depth = list(
    label = "Masa ciała ~ wysokość dzioba",
    x = "bill_depth_mm", y = "body_mass_g", default = 17, step = 0.5,
    unit = "mm wysokości dzioba",
    question = "Jaka będzie przewidywana średnia masa ciała pingwina o takiej wysokości dzioba?"
  )
)
...
ch1_ui <- list(
  ...
  content = tagList(
    ...
    tagList(
      ...
      p("palmerpenguins to dane o 333 pingwinach z trzech gatunków (Adelie,
        Chinstrap, Gentoo), zmierzonych na archipelagu Palmer u wybrzeży
        Antarktydy. Każdy wiersz to jeden pingwin, każda kolumna — jeden
        zmierzony parametr: długość i wysokość dzioba, długość płetwy, masa
        ciała, płeć oraz gatunek i wyspa. To dane, na których biolodzy badają,
        jak morfologia różni się między gatunkami i płciami."),
    ...
    lc_h2("ch1-caschool", "Regresja na danych pingwinów"),

    figure_panel(
      label = "Ryc. 1.4", title = "Pingwiny: od outputu do interpretacji",
      full_width = TRUE,
      fluidRow(
        column(4,
          ...
          selectInput("ch1_cas_x", "Zmienna X:",
            choices = c(
              "Długość płetwy (flipper_length_mm)" = "flipper_length_mm",
              "Długość dzioba (bill_length_mm)" = "bill_length_mm",
              "Wysokość dzioba (bill_depth_mm)" = "bill_depth_mm",
              "Masa ciała (body_mass_g)" = "body_mass_g",
              "Płeć (sex)" = "sex"
            ),
...
ch1_server <- function(input, output, session) {
  ...
  ch1_cas_model <- reactive({
    ...
    if (identical(input$ch1_cas_x, "sex")) {
      df <- .cas_data
      df$sex01 <- ifelse(df$sex == "male", 1, 0)
      lm(as.formula(paste(input$ch1_cas_y, "~ sex01")), data = df)
    } else {
  ...
  zoom_plot_server("ch1_cas_plot", reactive({
    ...
    if (identical(input$ch1_cas_x, "sex")) {
      ...
      ggplot(df, aes(x = sex, y = .data[[input$ch1_cas_y]])) +
        ...
        geom_crossbar(data = pred_df,
                      aes(x = sex, y = pred, ymin = pred, ymax = pred),
                      inherit.aes = FALSE,
  ...
  output$ch1_cas_table <- renderUI({
    ...
    coefs$term <- ifelse(coefs$term == "(Intercept)", "wyraz wolny",
                         ifelse(coefs$term == "sex01", "sex: samiec vs samica", input$ch1_cas_x))
  ...
  output$ch1_cas_answer <- renderUI({
    ...
    if (p_val < 0.05) {
      lc_feedback(type = "ok", style = "margin-top: 12px;",
        ...
        if (identical(input$ch1_cas_x, "sex")) {
          sprintf("tak, %s istotnie przewiduje %s. Samce różnią się od samic średnio o %.3f, p = %.3g.",
      ...
      lc_feedback(type = "warning", style = "margin-top: 12px;",
        tags$strong("Odpowiedź: "),
        if (identical(input$ch1_cas_x, "sex")) {
          sprintf("nie mamy podstaw, by uznać różnicę między samcami i samicami w %s za istotną: b1 = %.3f, p = %.3g.",
                  y_label, b1, p_val)
        } else {
      ...
      return(tagList(
        ...
        lc_formula_box(
          withMathJax(helpText(sprintf(
            "$$\\hat{Y} = %.2f %+ .2f \\cdot X_{\\text{samiec}}$$",
            b0, b1
          ))),
          p(tags$strong("Kodowanie: "), "samica = 0, samiec = 1."),
          withMathJax(helpText(sprintf(
            "$$\\text{samica: } \\hat{Y} = %.2f %+ .2f \\cdot 0 = %.2f$$",
            b0, b1, y0
          ))),
          withMathJax(helpText(sprintf(
            "$$\\text{samiec: } \\hat{Y} = %.2f %+ .2f \\cdot 1 = %.2f$$",
            b0, b1, y1
          )))
        ),
        lc_feedback(type = "info", style = "margin-top: 10px;",
          p(tags$strong("Interpretacja: "),
            paste0("w tym kodowaniu b₀ to średni przewidywany ", y_label,
                   " dla samic, a b₁ to różnica samce minus samice."))
        )
      ))
    }
