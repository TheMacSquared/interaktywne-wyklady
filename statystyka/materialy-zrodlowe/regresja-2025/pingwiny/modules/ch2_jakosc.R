...
.ch2_resid_specs <- list(
  mass_flipper = list(
    label = "Masa ciała ~ długość płetwy (model dobrze działa)",
  ...
  billdepth_billlength = list(
    label = "Wysokość dzioba ~ długość dzioba (ukryty gatunek)",
    x = "bill_length_mm", y = "bill_depth_mm",
    verdict = "warning",
    title = "Wzorzec w resztach: model ignoruje gatunek",
    comment = "Reszty rozpadają się na trzy chmury — po jednej na gatunek.
              Pojedyncza prosta miesza trzy grupy, więc pominięty czynnik
              (gatunek) zostawia w resztach wyraźny ślad. To zapowiedź
              regresji wielorakiej."
  ),
  mass_billdepth = list(
    label = "Masa ciała ~ wysokość dzioba (słaby model, ale bez wzorca)",
    x = "bill_depth_mm", y = "body_mass_g",
    verdict = "info",
    title = "Słaby model, ale uczciwy",
    comment = "Reszty są duże, bo R² jest niskie — wysokość dzioba słabo
              przewiduje masę ciała. Ale wzorzec sam w sobie jest losowy.
              To rzetelny model: po prostu niewiele tłumaczy."
  ),
  billlen_flipper = list(
    label = "Długość dzioba ~ długość płetwy (przyzwoity)",
...
.ch2_rmse_specs <- list(
  mass_flipper = list(
    label = "Masa ciała ~ długość płetwy (najlepsze dopasowanie)",
    x = "flipper_length_mm", y = "body_mass_g"
  ),
  billlen_flipper = list(
    label = "Długość dzioba ~ długość płetwy",
    x = "flipper_length_mm", y = "bill_length_mm"
  ),
  mass_billdepth = list(
    label = "Masa ciała ~ wysokość dzioba",
    x = "bill_depth_mm", y = "body_mass_g"
  ),
  billdepth_billlength = list(
    label = "Wysokość dzioba ~ długość dzioba",
    x = "bill_length_mm", y = "bill_depth_mm"
  )
)
...
ch2_ui <- list(
  ...
  content = tagList(
    ...
    figure_panel(
      label = "Ryc. 2.1", title = "Reszty na danych pingwinów",
      full_width = TRUE,
      fluidRow(
        column(4,
          helpText("Cztery scenariusze na tych samych pingwinach. Zobacz, jak różny model zachowuje się na tych samych danych."),
          selectInput("ch2_resid_case", "Model:",
            choices = .ch2_resid_choices,
            selected = "billdepth_billlength"
          ),
    ...
    tagList(
      p("R² jest miarą względną — daje wartość między 0 a 1, ale nie mówi
        ...
        jeśli model przewiduje masę ciała, czy myli się o 50 gramów czy o 500?"),
      ...
      p("Liczone w jednostkach Y. Jeśli Y to masa ciała w gramach (rozrzut
        ~2700–6300 g), a RMSE wyszło 400, znaczy: typowa pomyłka modelu to
        ±400 g. To dużo czy mało? Zależy od skali."),
      p(tags$strong("Złota zasada: "),
        "RMSE zawsze porównuj z rozrzutem Y. RMSE = 400 dla zmiennej o rozrzucie
        ~3600 g to nie najgorzej. RMSE = 400 dla zmiennej w skali 0–500 to
        katastrofa.")
    ),

    figure_panel(
      label = "Ryc. 2.3", title = "RMSE i zakres Y na danych pingwinów",
      full_width = TRUE,
      fluidRow(
        column(4,
    ...
    tagList(
      ...
      p("Ekstrapolacja jest niebezpieczna, bo linia wygląda pewnie
        ...
        jest nadal liniowa."),
      p("Przykład: model przewiduje masę ciała pingwina na podstawie długości
        płetwy. Dane obejmują płetwy 172–231 mm. Co jeśli zapytamy o płetwę
        260 mm? Prosta obliczy wynik — ale nie ma żadnych danych w tym
        przedziale, żeby to potwierdzić.")
    ),
    ...
    figure_panel(
      ...
      fluidRow(
        column(4,
          ...
          sliderInput("ch2_extrap_x", "Długość płetwy (mm):",
            min = 150, max = 270, value = 200, step = 1),
...
ch2_server <- function(input, output, session) {
  ...
  ch2_resid_spec <- reactive({
    ...
    if (is.null(case)) case <- "billdepth_billlength"
  ...
  ch2_rmse_spec <- reactive({
    ...
    if (is.null(case)) case <- "mass_flipper"
    .ch2_rmse_specs[[case]]
  })
  ...
  .ch2_extrap_model <- lm(body_mass_g ~ flipper_length_mm, data = .cas_data)
  .ch2_extrap_x_range <- range(.cas_data$flipper_length_mm)

  zoom_plot_server("ch2_extrap_plot", reactive({
    x_val <- input$ch2_extrap_x
    if (is.null(x_val)) x_val <- 200
    x_obs <- .cas_data$flipper_length_mm
    y_obs <- .cas_data$body_mass_g
    x_range <- .ch2_extrap_x_range
    x_grid <- seq(min(150, x_val - 2), max(270, x_val + 2), length.out = 300)
    df_line <- data.frame(
      flipper_length_mm = x_grid,
      body_mass_g = predict(.ch2_extrap_model, newdata = data.frame(flipper_length_mm = x_grid)),
      outside = x_grid < x_range[1] | x_grid > x_range[2]
    )
    y_pred <- predict(.ch2_extrap_model, newdata = data.frame(flipper_length_mm = x_val))
  ...
  output$ch2_extrap_verdict <- renderUI({
    ...
    if (in_range) {
      lc_feedback(type = "ok", style = "margin-top: 12px;",
        ...
        p(sprintf("Predykcja: %.0f g. Jesteśmy wewnątrz zakresu danych — predykcja ma sens.", y_pred))
      )
    } else {
      lc_feedback(type = "warning", style = "margin-top: 12px;",
        tags$strong("Ekstrapolacja!"),
        p(sprintf("Predykcja: %.0f g. Jesteśmy %.0f%% zakresu danych poza granicą — brak gwarancji.", y_pred, dist_pct))
      )
    }
  })
  ...
  output$ch2_extrap_stats <- renderUI({
    ...
    lc_stat_grid(
      lc_stat_box("X podany", x_val,
                  caption = "mm płetwy",
                  color = unname(upwr_cat["niebo"])),
      lc_stat_box("Predykcja", round(y_pred, 0),
                  caption = "g masy ciała",
                  color = unname(upwr_cat["bursztyn"])),
