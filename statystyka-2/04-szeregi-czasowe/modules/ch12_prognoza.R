...
ch12_ui <- list(
  ...
  content = tagList(
    ...
    figure_panel(
      label = "Ryc. 12.3", title = "Prognoza vs. ukryte dane syntetyczne",
      full_width = TRUE,
      fluidRow(
        column(4,
...
ch12_server <- function(input, output, session) {
  ...
  zoom_plot_server("ch12_forecast_plot", reactive({
    ...
    df_obs <- data.frame(t = t_obs, value = as.numeric(ts_obj), typ = "Scenariusz syntetyczny")
  ...
  zoom_plot_server("ch12_reveal_plot", reactive({
    ...
    df_actual <- data.frame(t = n_train + seq_len(n_test), value = actual,     typ = "Dane syntetyczne (ukryte)")
    ...
    kolory    <- c("Dane uczące" = upwr_secondary,
                   "Dane syntetyczne (ukryte)" = upwr_cat["szalwia"],
                   "Prognoza" = upwr_accent)
    linie     <- c("Dane uczące" = "solid",
                   "Dane syntetyczne (ukryte)" = "solid",
                   "Prognoza" = "dashed")
