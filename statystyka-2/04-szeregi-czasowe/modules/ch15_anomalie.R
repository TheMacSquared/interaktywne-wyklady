  ...
  content = tagList(
    ...
    lc_chapter_hero(
      ...
      lead   = "Niektóre zdarzenia nie są 'szumem' — są prawdziwymi zerwaniami struktury.
                Na syntetycznych danych odtworzymy scenariusze spadku noclegów i skoku cen pszenicy. Jak model sobie z tym radzi?"
    ),

    lc_h2("ch15-covid", "COVID-19 w noclegach — przykład anomalii"),

    tagList(
      lc_p("W syntetycznym scenariuszu w 2020 r. dodaliśmy głęboki spadek liczby noclegów. To nie sezonowość,
      ...
      margin_callout(label = "Kiedy to anomalia, a kiedy nowy reżim?", color = "wskazowka",
        "Anomalia: jednorazowy wstrząs po którym szereg wraca do poprzedniego wzorca (np. lockdown 2020).
         ...
         Syntetyczny scenariusz pszenicy w 2022 r. ilustruje taki problem."
      )
    ),
    ...
    tagList(
      lc_p("W naszym syntetycznym scenariuszu w 2022 r. dodaliśmy gwałtowny skok cen pszenicy, a potem częściowy spadek. To przykład inspirowany zakłóceniami rynku: badamy reakcję modelu na zaprojektowany szok, nie mierzymy skutków konkretnego wydarzenia historycznego.")
    ),
