...
.ts_datasets <- list(
  ...
  noclegi = list(
    ...
    desc   = "Miesięczna liczba noclegów w Polsce (2010–2023). Silna sezonowość wakacyjna i symulowany dołek inspirowany pandemią."
  ),
  sprzedaz = list(
...
plot_forecast_gg <- function(ts_data, fc, value_label = "wartość",
  ...
  n <- length(ts_data)
  t_obs <- seq_len(n)
  df_obs <- data.frame(t = t_obs, value = as.numeric(ts_data), type = "Scenariusz syntetyczny")
  ...
  p <- ggplot(df_all, aes(x = t, y = value, color = type, linetype = type)) +
    ...
    scale_color_manual(
      values = c("Scenariusz syntetyczny" = upwr_secondary,
                 "Prognoza"          = upwr_accent),
      name = NULL
    ) +
    scale_linetype_manual(
      values = c("Scenariusz syntetyczny" = "solid",
                 "Prognoza"          = "dashed"),
      name = NULL
    ) +
...
# Podpis pozostaje widoczny także po powiększeniu wykresu lub zapisaniu ggplot.
ts_mark_synthetic <- function(plot) {
  if (inherits(plot, "ggplot")) {
    old_caption <- plot$labels$caption
    plot <- plot + ggplot2::labs(caption = paste(
      c(old_caption, "Dane syntetyczne · scenariusz dydaktyczny"), collapse = "\n"
    ))
  }
  plot
}

ts_base_zoom_plot_server <- zoom_plot_server
zoom_plot_server <- function(id, plot_fn, alt = "Wykres ilustrujący omawiane zagadnienie") {
  ts_base_zoom_plot_server(id, shiny::reactive(ts_mark_synthetic(plot_fn())),
    alt = paste(alt, "Dane syntetyczne."))
}
