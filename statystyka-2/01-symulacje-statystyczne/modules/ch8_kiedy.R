  ...
  content = tagList(
    ...
    tags$table(class = "decision-table",
      ...
      tags$tbody(
        ...
        tags$tr(
          ...
          tags$td("Bez normalności; przetasowanie wymaga wymienności pod H₀")
        ),
        tags$tr(
    ...
    lc_feedback(type = "warning",
      ...
      " Mann-Whitney U testuje inne H₀ niż test t (przesunięcie rozkładu,
       nie różnica średnich). Bootstrap może dotyczyć różnicy średnich,
       ale wymaga właściwego schematu losowania, a test — odtworzenia H₀.
       Nie zamieniaj metod bez sprawdzenia, jakie pytanie rozstrzygają."
    ),
    ...
    tagList(
      p(tags$b("Test permutacyjny"), ": przetasowuje etykiety grup.
         ...
         (np. identyczne rozkłady w niezależnych grupach)."),
...
ch8_server <- function(input, output, session) {
  ...
  ch8_answers <- list(
    ...
    s3 = list(
      ...
      body  = "Brak normalności sam nie uzasadnia permutacji. Przetasowanie etykiet wymaga wymienności pod H₀; sama równość średnich przy różnych rozkładach nie wystarcza. Bootstrap musi zachować strukturę próby."
    ),
    s4 = list(
      type  = "ok",
      title = "Zalecany: Bootstrap CI dla r lub test permutacyjny korelacji",
      body  = "Najpierw sprawdź obserwacje odstające. Bootstrap i permutacje nie usuwają wrażliwości korelacji Pearsona na takie obserwacje."
    ),
    s5 = list(
      type  = "ok",
