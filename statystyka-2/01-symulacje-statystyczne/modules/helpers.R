...
# Korekta Monte Carlo: obserwowany układ jest dodatkową konfiguracją.
# Przedział dotyczy błędu losowania symulacji, nie niepewności parametru populacji.
simulation_p_summary <- function(extreme) {
  stopifnot(is.logical(extreme), length(extreme) > 0L, !anyNA(extreme))
  B <- length(extreme)
  b <- sum(extreme)
  interval <- c(
    lower = if (b == 0) 0 else qbeta(.025, b, B - b + 1),
    upper = if (b == B) 1 else qbeta(.975, b + 1, B - b)
  )
  list(p_value = (b + 1) / (B + 1), B = B, extreme_count = b,
       resolution = 1 / (B + 1), mc_interval = interval)
}

simulation_precision_note <- function(result) {
  sprintf(
    "Ekstremalne wyniki: %d z %d. p = (b + 1)/(B + 1); najmniejsza możliwa wartość: %.4g. Przedział 95%% dla prawdopodobieństwa ogona wynikający z losowania symulacji: [%.4g; %.4g]. Więcej losowań zmniejsza ten błąd symulacyjny.",
    result$extreme_count, result$B, result$resolution,
    result$mc_interval[1], result$mc_interval[2]
  )
}
...
run_permutation_test_twosample <- function(data, B = 1000,
  ...
  tolerance <- 64 * .Machine$double.eps * max(1, abs(obs_diff))
  extreme <- switch(match.arg(alternative, c("two.sided", "greater", "less")),
    "two.sided" = abs(perm_diffs) >= abs(obs_diff) - tolerance,
    "greater"   = perm_diffs >= obs_diff - tolerance,
    "less"      = perm_diffs <= obs_diff + tolerance
  )
  precision <- simulation_p_summary(extreme)

  list(
    perm_diffs    = perm_diffs,
    observed_diff = obs_diff,
    p_value       = precision$p_value,
    B = precision$B, extreme_count = precision$extreme_count,
    resolution = precision$resolution, mc_interval = precision$mc_interval,
...
run_permutation_test_correlation <- function(data, B = 1000,
  ...
  tolerance <- 64 * .Machine$double.eps * max(1, abs(obs_r))
  extreme <- switch(match.arg(alternative, c("two.sided", "greater", "less")),
    "two.sided" = abs(perm_cors) >= abs(obs_r) - tolerance,
    "greater"   = perm_cors >= obs_r - tolerance,
    "less"      = perm_cors <= obs_r + tolerance
  )
  precision <- simulation_p_summary(extreme)
...
run_mc_null <- function(observed_data, scenario, B = 5000, seed = NULL) {
  ...
  precision <- simulation_p_summary(abs(null_stats) >= abs(obs_stat))

  list(
    null_stats    = null_stats,
    p_value_mc    = precision$p_value,
    B = precision$B, extreme_count = precision$extreme_count,
    resolution = precision$resolution, mc_interval = precision$mc_interval,
