...
ch10_server <- function(input, output, session) {
  ...
  observeEvent(input$ch10_zyw_ans2, {
    output$ch10_zyw_sol2 <- renderUI({
      lc_feedback(type = "ok",
        tags$strong("Rozwiązanie:"),
        p("Test permutacyjny porównuje średnie, ale przetasowanie wymaga wymienności pod H₀, np. identycznych rozkładów w niezależnych grupach. Sama równość średnich nie wystarczy. Mann–Whitney porównuje rangi i tendencję do większych wartości w jednej grupie."),
        p("Różne statystyki testowe mogą dawać różne p-wartości. Dobieraj metodę do pytania i struktury danych; przy bootstrapie różnicy średnich losuj osobno wewnątrz niezależnych grup.")
      )
    })
  })
  ...
  observeEvent(input$ch10_zyw_ans3, {
    output$ch10_zyw_sol3 <- renderUI({
      lc_feedback(type = "ok",
        ...
        p("W bootstrapie korelacji losujemy całe pary (białko, tekstura), zachowując związek wewnątrz pary. Ten schemat zakłada niezależność par i reprezentatywność próby; nie wymaga normalności, ale nie usuwa wpływu obserwacji odstających.")
      )
    })
  })
